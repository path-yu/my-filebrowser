    ...
    if (
      ...
      if (opts?.permission) {
        getPermission("clipboard-write")
          .then(() =>
            writeToClipboard(data).catch(() => {
              // Clipboard API 失败（如文档失焦/权限被拒）时降级到旧方法
              legacyCopyText(data, resolve, reject);
            })
          )
          .catch(() => legacyCopyText(data, resolve, reject));
      } else {
        writeToClipboard(data)
          .then(resolve)
          .catch(() => {
            // 同上：降级到 execCommand 方案
            legacyCopyText(data, resolve, reject);
          });
      }
    } else if (document.queryCommandSupported("copy") && data.text) {
      legacyCopyText(data, resolve, reject);
    } else {
      reject(
...
function legacyCopyText(
  ...
  if (!data.text) {
    reject(new Error("Legacy copying method only supports text!"));
    return;
  }
  if (!document.queryCommandSupported("copy")) {
    reject(new Error("Legacy copying method is not supported!"));
    return;
  }
