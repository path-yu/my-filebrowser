...
// 触发 mock 数据与 mock auth 注入（仅当 ?mockTime=1 或 localStorage.mockTime==="1" 时才生效）。
// 必须在 router.beforeEach 执行之前 import，保证进入 /files/ 之前 authStore 已写入虚拟用户，
// 不会被守卫重定向到 /login。
import "./api/mockTimeFilter";
...
// The global mixin that exposed every component instance on its DOM element
// (el.__vue__) was removed: it handed attackers a live handle to the app's
// reactive state from any XSS or console access. ListingItem now publishes
// data-url/data-dir attributes instead, and consumers read dataset.*.
