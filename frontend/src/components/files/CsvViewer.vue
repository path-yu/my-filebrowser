...
// 下拉选项用字符串值（组件按 === 匹配），再映射为解析器所需的分隔符数组
const separatorChoice = ref("both");
...
const columnSeparator = computed<string[]>(
  () => SEPARATOR_MAP[separatorChoice.value] ?? SEPARATOR_MAP.both,
);

const separatorOptions = computed(() => [
  { value: "comma", label: t("files.csvSeparators.comma") },
  { value: "semicolon", label: t("files.csvSeparators.semicolon") },
  { value: "both", label: t("files.csvSeparators.both") },
]);
