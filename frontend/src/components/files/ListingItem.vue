<template>
  ...
  >
    ...
    <div>
      <p class="name">
        <template v-for="(seg, i) in nameSegments" :key="`${i}-${seg.text}`">
    ...
    <!-- 产品编号 subtitle：位于整行下方，跨所有列，点击复制 -->
    <p
      v-if="productCode"
      class="product-code"
      role="button"
      tabindex="0"
      :aria-label="t('files.productCodeCopy', '点击复制产品编号')"
      :title="t('files.productCodeCopy', '点击复制产品编号')"
      @click.stop="copyProductCode"
      @keydown.enter.prevent.stop="copyProductCode"
    >
      {{ productCode }}
    </p>
  </div>
...
const $showSuccess = inject<IToastSuccess | null>("$showSuccess", null);
...
/** 点击产品编号 subtitle 复制编号 */
const copyProductCode = async () => {
  if (!props.productCode) return;
  try {
    await copy({ text: props.productCode });
    $showSuccess?.(t("success.productCodeCopied", "产品编号已复制！"));
  } catch (e) {
    try {
      await copy({ text: props.productCode }, { permission: true });
      $showSuccess?.(t("success.productCodeCopied", "产品编号已复制！"));
    } catch (e2) {
