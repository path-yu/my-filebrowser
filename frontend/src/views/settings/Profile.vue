...
/** 个人主题偏好：
 *  - 空串 → 使用系统默认（跟随系统设置的 branding.theme 或媒体查询）
 *  - light → 强制浅色
 *  - dark → 强制深色
 *  立即调用 setTheme() 实时生效，刷新后通过 getTheme() 从 localStorage 还原。 */
const theme = ref<UserTheme>(getTheme() || "");
...
/** 主题切换：即时切换 UI（写入 localStorage + html/root 类名），保存按钮再走 API 更新后端 */
watch(theme, (t) => {
  setTheme(t);
});
