...
func tusPostHandler(cache UploadCache) handleFunc {
	return withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
		...
		case errors.Is(err, afero.ErrFileNotFound):
			// path.Dir (not filepath.Dir): the virtual path is URL-style, and
			// on Windows filepath.Dir would yield a backslash-separated string
			// that user rules written with forward slashes never match —
			// silently bypassing deny rules on the parent directory.
