...
var commandsHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	// FullPath is a purely lexical join: a URL path like "/..\\..\\x" (which
	// path.Clean does not neutralize on Windows) or an in-scope symlink whose
	// target lives outside the user's scope would make the spawned command run
	// with its working directory outside the user's scope. Validate the path
	// through the scoped filesystem (whose guard resolves symlinks and rejects
	// escapes) and the rule checker before handing it to exec.
	...
	if _, err := d.user.Fs.Stat(r.URL.Path); err != nil {
		wsErr(conn, r, http.StatusForbidden, err)
		return 0, nil
	}
