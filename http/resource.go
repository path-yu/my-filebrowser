...
var resourceGetHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	if file.IsDir {
		...
		file.ApplySort()
		// 时间筛选：只过滤文件条目，子目录保持可见以便导航
		if after, before, ok := parseModifiedRange(r.URL.Query()); ok {
			filtered := make([]*files.FileInfo, 0, len(file.Items))
			for _, it := range file.Items {
				if inModifiedRange(it.IsDir, it.ModTime, after, before) {
					filtered = append(filtered, it)
				}
			}
			file.Items = filtered
			file.NumFiles = len(filtered)
		}
...
var resourceGetRecursiveHandler = withUser(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	err = afero.Walk(d.user.Fs, rootPath, func(fPath string, info os.FileInfo, err error) error {
		...
		if applyFilter && !inModifiedRange(info.IsDir(), info.ModTime(), after, before) {
			if info.IsDir() {
				return nil // 不跳过子目录：目录下的文件可能仍在时间范围内
			}
			return nil
		}
...
// parseModifiedRange 从 query 中解析 modified_after / modified_before（Unix 毫秒）。
// 当两个参数都为空时返回 ok=false，表示调用方无需做时间过滤。
func parseModifiedRange(q url.Values) (after time.Time, before time.Time, ok bool) {
	a := strings.TrimSpace(q.Get("modified_after"))
	b := strings.TrimSpace(q.Get("modified_before"))
	if a == "" && b == "" {
		return time.Time{}, time.Time{}, false
	}
	parseMillis := func(s string) (time.Time, error) {
		if s == "" {
...
// inModifiedRange 判断文件修改时间是否在 [after, before] 范围内。
// dir 一律返回 true（子目录不过滤，用户需要它们保持可见用于导航）。
func inModifiedRange(isDir bool, modTime time.Time, after, before time.Time) bool {
	if isDir {
		return true
	}
