...
type data struct {
	*runner.Runner
	...
	// userOriginalFs is set only for public share handlers to the user's
	// original, non-scoped Fs. withHashFile rebases d.user.Fs onto the share
	// root via ScopedFs, but some handlers (e.g. doc convert) need the real
	// on-disk path which requires resolving via the original Fs.
	userOriginalFs afero.Fs
}
